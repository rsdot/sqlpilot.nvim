...
function M.sql_create_autocmd(dbms)
  ...
  local sqlpilot_buffer_settings = function()
    ...
    if vim.b.match_words == nil then
      vim.b.match_words =
        [[{:},(:),[:],\(\<CASE\>\|\<BEGIN\>\)\(\s\+TRAN\)\@!:\<WHEN\>:\<THEN\>:\<END\>,\(\<UPDATE\>\|\<SELECT\>\|\<DELETE\>\):\<FROM\>:\<WHERE\>,\<INSERT\>:\<VALUES\>]]
    end
  ...
  vim.api.nvim_create_autocmd(
    "Filetype",
    { group = "sqlpilot", pattern = "sql", callback = sqlpilot_buffer_settings }
  )
  vim.api.nvim_create_autocmd(
    "Filetype",
    { group = "sqlpilot", pattern = "sql", command = [[match Sql_Semicolon_WhitespaceEOL /\(;\s*\|\s\+\)$/]] }
  )
  -- stylua: ignore end
end
